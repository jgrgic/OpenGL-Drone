...
CubeMesh newDronePart2(Vector3D entireTranslate, Vector3D scale, Vector3D setup, float droneRotation, float propellerSpin, float droneRotationOrigin) {
	...
	cube.angle = droneRotation;
	cube.aroundOrigin = droneRotationOrigin;
	cube.tx = entireTranslate.x;
	cube.ty = entireTranslate.y;
	cube.tz = entireTranslate.z;
	cube.sfx = scale.x;
	cube.sfy = scale.y;
	cube.sfz = scale.z;
	cube.sx = setup.x;
...
void drawDrone(CubeMesh *cube) {
	...
	glTranslatef(cube->tx, cube->ty, cube->tz);
	glRotatef(cube->angle, 0.0, 1.0, 0.0);
	glTranslatef(cube->sx, cube->sy, cube->sz);
	glRotatef(cube->propeller, 0.0, 1.0, 0.0);
	glScalef(cube->sfx, cube->sfy, cube->sfz);
...
void drawDrone2(CubeMesh *cube) {
	...
	glRotatef(cube->aroundOrigin, 0.0, 1.0, 0.0);
	glTranslatef(cube->tx, cube->ty, cube->tz);
	glTranslatef(cube->sx, cube->sy, cube->sz);
	glRotatef(cube->propeller, 0.0, 1.0, 0.0);
	glScalef(cube->sfx, cube->sfy, cube->sfz);
